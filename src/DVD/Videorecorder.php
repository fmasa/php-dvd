...
use DVD\Event\AfterHttpRequestEvent;
use DVD\Event\AfterPlaybackEvent;
use DVD\Event\BeforeHttpRequestEvent;
use DVD\Event\BeforePlaybackEvent;
use DVD\Event\BeforeRecordEvent;
...
class Videorecorder
{
    ...
    /**
     * @var DVDFactory Factory which can create instances and resolve dependencies.
     */
    protected $factory;
     ...
     * @param DVDFactory    $factory Factory which can create instances and resolve dependencies.
     */
    public function __construct(Configuration $config, HttpClient $client, DVDFactory $factory)
    {
        $this->config = $config;
        $this->client = $client;
    ...
    {
        Assertion::true($this->isOn, 'Please turn on DVD before ejecting a cassette, use: DVD::turnOn().');
        $this->cassette = null;
    }
     ...
     *
     * @return void
     * @throws DVDException If videorecorder is turned off when inserting a cassette.
     */
    public function insertCassette($cassetteName)
    {
        Assertion::true($this->isOn, 'Please turn on DVD before inserting a cassette, use: DVD::turnOn().');
    ...
    public function handleRequest(Request $request)
    {
        if ($this->cassette === null) {
            throw new \BadMethodCallException(
                ...
                . "DVD::insertCassette('name');"
            );
        }

        $event = new BeforePlaybackEvent($request, $this->cassette);
        $this->dispatch(DVDEvents::BEFORE_PLAYBACK, $event);
        ...
        if (!empty($response)) {
            ...
            $this->dispatch(DVDEvents::AFTER_PLAYBACK, $event);
            return $response;
        }

        if (DVD::MODE_NONE === $this->config->getMode()
            || DVD::MODE_ONCE === $this->config->getMode()
        ...
        $this->dispatch(DVDEvents::BEFORE_HTTP_REQUEST, new BeforeHttpRequestEvent($request));
        $response = $this->client->send($request);
        $this->dispatch(DVDEvents::AFTER_HTTP_REQUEST, new AfterHttpRequestEvent($request, $response));

        $this->dispatch(DVDEvents::BEFORE_RECORD, new BeforeRecordEvent($request, $response, $this->cassette));
